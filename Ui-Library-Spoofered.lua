...
function API.AddSlider(text, min, max, default, callback)
    ...
    --==============================================
    local SettingsAPI = CreateTab("Configurações")
    local SettingsTab = Tabs[#Tabs]
    SettingsTab.button.Visible = false

    SettingsAPI.AddSection("Perfis de configuração")
    SettingsAPI.AddParagraph(
        "Configurações salvas",
        "Crie perfis com os estados atuais dos controles. O perfil selecionado pode ser carregado a qualquer momento."
    )

    local ProfileNameBox = SettingsAPI.AddInput("Nome da configuração", "Ex.: Minha configuração", nil)
    ...
    local ProfileSelector = new("TextButton", {
        ...
        BorderSizePixel = 0,
        Font = CONFIG.Font,
        Text = "  " .. (selectedProfile ~= "" and selectedProfile or "Nenhuma configuração"),
        TextColor3 = THEME.Text,
        TextSize = 13,
        TextXAlignment = Enum.TextXAlignment.Left,
        AutoButtonColor = false,
    ...
    SettingsAPI.AddButton("Criar configuração", function()
    ...
    SettingsAPI.AddButton("Carregar configuração automaticamente", function()
        if selectedProfile ~= "" then
            applyProfile(selectedProfile)
        end
    end)

    SettingsAPI.AddButton("Deletar configuração", function()
        ...
        ProfileSelector.Text = "  Nenhuma configuração"
        refreshProfileList()
    end)

    if CONFIG.AutoLoadLastConfig and selectedProfile ~= "" and ProfileData.Profiles[selectedProfile] then
        task.defer(function()
            applyProfile(selectedProfile)
        end)
    end
