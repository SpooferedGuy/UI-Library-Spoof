...
local function Build(userConfig)
    ...
    local ProfileData = loadSavedConfig(ProfileFileName)
    if type(ProfileData.Profiles) ~= "table" then
        ProfileData.Profiles = {}
    end
    ProfileData.LastSelected = tostring(ProfileData.LastSelected or "")
    ProfileData.AutoLoad = ProfileData.AutoLoad == true
    ...
    local function saveValue(key, value)
        ...
        if CONFIG.loadSaveConfig and not ApplyingProfile then
            saveConfig(ConfigFileName, RuntimeConfig)
        end
    end

    local function cloneConfig(data)
        local out = {}
    ...
    local function saveProfiles()
        return saveConfig(ProfileFileName, ProfileData)
    end

    local function applyProfile(name)
        local profile = ProfileData.Profiles[name]
        if type(profile) ~= "table" then
            return false
        end

        ApplyingProfile = true
        SavedConfig = cloneConfig(profile)
        ...
        ProfileData.LastSelected = name
        saveProfiles()

        if CONFIG.loadSaveConfig then
            saveConfig(ConfigFileName, RuntimeConfig)
        end
        return true
    end
...
local CloseBtn = new("TextButton", {
    ...
    local function CreateTab(name)
        ...
        function API.AddToggle(text, default, callback)
            ...
            RuntimeConfig[configKey] = state
            ControlRegistry[configKey] = function(newValue)
                state = newValue == true
                switch.BackgroundColor3 = state and THEME.Accent or THEME.Tertiary
        ...
        function API.AddInput(text, placeholder, callback)
            ...
            RuntimeConfig[configKey] = box.Text
            ControlRegistry[configKey] = function(newValue)
...
function API.AddSlider(text, min, max, default, callback)
        ...
        function API.AddDropdown(text, options, default, callback)
            ...
            RuntimeConfig[configKey] = selected
            ControlRegistry[configKey] = function(newValue)
                selected = newValue
    ...
    local selectedProfile = ProfileData.LastSelected
    local ProfileSelectorFrame = new("Frame", {
    ...
    corner(ProfileSelectorFrame, 6); padding(ProfileSelectorFrame, 8)

    new("TextLabel", {
        Parent = ProfileSelectorFrame,
        BackgroundTransparency = 1,
    ...
    local ProfileSelector = new("TextButton", {
        Parent = ProfileSelectorFrame,
        ...
        BorderSizePixel = 0,
        Font = CONFIG.Font,
        Text = "  " .. (selectedProfile ~= "" and selectedProfile or "No configuration"),
        TextColor3 = THEME.Text,
        TextSize = 13,
        TextXAlignment = Enum.TextXAlignment.Left,
        AutoButtonColor = false,
    ...
    corner(ProfileSelector, 6)

    local ProfileList = new("Frame", {
        Parent = ProfileSelectorFrame,
    ...
    corner(ProfileList, 6); stroke(ProfileList, THEME.Stroke); padding(ProfileList, 4)
    new("UIListLayout", {
        Parent = ProfileList,
    ...
    local function refreshProfileList()
        ...
        local names = {}
        for name in pairs(ProfileData.Profiles) do
        ...
        for _, name in ipairs(names) do
            local item = new("TextButton", {
                ...
                BackgroundColor3 = THEME.Secondary,
                BackgroundTransparency = name == selectedProfile and 0 or 1,
                BorderSizePixel = 0,
                Font = CONFIG.Font,
                Text = "  " .. name,
                TextColor3 = THEME.Text,
            ...
            item.MouseButton1Click:Connect(function()
                ...
                ProfileSelector.Text = "  " .. name
                ProfileList.Visible = false
                ProfileList.Size = UDim2.new(1, 0, 0, 0)
                ProfileSelectorFrame.Size = UDim2.new(1, 0, 0, 52)
                refreshProfileList()
            end)
        end
    end
    ...
    ProfileSelector.MouseButton1Click:Connect(function()
        if ProfileList.Visible then
            tween(ProfileList, 0.15, { Size = UDim2.new(1, 0, 0, 0) })
            task.delay(0.15, function() ProfileList.Visible = false end)
            ProfileSelectorFrame.Size = UDim2.new(1, 0, 0, 52)
            return
        end

        local count = 0
        for _ in pairs(ProfileData.Profiles) do count += 1 end
        local target = math.min(count * 26 + 8, 150)
        ProfileList.Visible = true
        tween(ProfileList, 0.2, { Size = UDim2.new(1, 0, 0, target) })
        ProfileSelectorFrame.Size = UDim2.new(1, 0, 0, 52 + target)
    end)

    SettingsAPI.AddButton("Create Configuration", function()
        local name = ProfileNameBox.Text:gsub("^%s+", ""):gsub("%s+$", "")
        if name == "" then return end

        ProfileData.Profiles[name] = cloneConfig(RuntimeConfig)
        selectedProfile = name
        ProfileData.LastSelected = name
        saveProfiles()
    ...
    AutoLoadButton = SettingsAPI.AddButton(
        ProfileData.AutoLoad and "Auto Load: ON" or "Auto Load: OFF",
        function()
            if selectedProfile == "" then return end
            ProfileData.AutoLoad = not ProfileData.AutoLoad
            ProfileData.LastSelected = selectedProfile
            saveProfiles()
            AutoLoadButton.Text = ProfileData.AutoLoad and "Auto Load: ON" or "Auto Load: OFF"
            if ProfileData.AutoLoad then
                applyProfile(selectedProfile)
            end
        end
    )

    SettingsAPI.AddButton("Disable All Settings", function()
        ApplyingProfile = true
        for key, setter in pairs(ControlRegistry) do
            if string.sub(key, 1, 7) == "Toggle_" then
                setter(false)
    ...
    SettingsAPI.AddButton("Delete Configuration", function()
        if selectedProfile == "" or not ProfileData.Profiles[selectedProfile] then return end

        ProfileData.Profiles[selectedProfile] = nil
        selectedProfile = ""
        ProfileData.LastSelected = ""
        ProfileData.AutoLoad = false
        saveProfiles()
        AutoLoadButton.Text = "Auto Load: OFF"

        ProfileSelector.Text = "  No configuration"
        refreshProfileList()
    end)

    if ProfileData.AutoLoad and selectedProfile ~= "" and ProfileData.Profiles[selectedProfile] then
        task.defer(function()
            applyProfile(selectedProfile)
        end)
    end
    ...
    return {
        ...
        LoadNamedConfig = function(name)
            return applyProfile(tostring(name or ""))
        end,
        DeleteConfig = function(name)
            name = tostring(name or "")
            if not ProfileData.Profiles[name] then return false end
            ProfileData.Profiles[name] = nil
            if ProfileData.LastSelected == name then
                ProfileData.LastSelected = ""
            end
            return saveProfiles()
        end,
